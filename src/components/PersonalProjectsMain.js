    ...
    render() {
        ...
        return (
            <div id="main" style={this.props.timeout ? { display: 'flex' } : { display: 'none' }}>
                ...
                <article id="t�dey" className={`${this.props.article === 't�dey' ? 'active' : ''} ${this.props.articleTimeout ? 'timeout' : ''}`} style={{ display: 'none' }}>
                    <h2 className="major">T�dey Physics</h2>
                    <span className="image main"><img src={pic01} alt="" /></span>
                    <h3>Summary</h3>
                    <p>2D physics and math libraries created in C++. The initial goal of T�dey was to have an independent 2D physics library that could be plugged into an engine and have everything the gameplay side of the engine would need for a basic 2D game. This meant efficient collision detection and resolution, a way of controlling whether certain object collide with others, and an event system that allows you to make events external to T�dey, but have them handled by the engine. I ended up also adding some more things that are not necessary to make a 2D game with physics, but they are nice to have. </p>
                    <h3>Serialization</h3>
                    <p>Further along in the project I started working on a Networking library and because of this added serialization of the physics world to T�dey. The majority of this process was simple enough, however serialization of the collision behaviours was tricky. The idea behind the collision behaviours is to allow programmers to inherit from the abstract class, decide what to add to it and choose which functions they would like to override; these functions were OnTriggerEnter, OnTriggerStay, OnTriggerExit, OnCollisionEnter, OnCollisionStay, and OnCollisionExit. They would then pass an instance of that class as a parameter when creating a Collider object, and the physics library would handle when these events are called. T�dey could serialize these objects no problem, but deserializing was a different question. I wanted the entire serialization process to be encapsulated in the physics library, but the library does not know what types had been defined outside of it. I ended up creating a very lightweight reflection system that held all the CollisionBehaviour child types that was accessed by their class name.</p>
