...
import tudeyLogo from '../images/PhysicsLogo.png'
        ...
        return (
            <div id="main" style={this.props.timeout ? { display: 'flex' } : { display: 'none' }}>
                ...
                <article id="tudey" className={`${this.props.article === 'tudey' ? 'active' : ''} ${this.props.articleTimeout ? 'timeout' : ''}`} style={{ display: 'none' }}>
                    ...
                    <h3>Serialization</h3>
                    <p>Further along in the project I started working on a Networking library and because of this added serialization of the physics world to Tudey. The majority of this process was simple enough, however serialization of the collision behaviours was tricky. The idea behind the collision behaviours is to allow programmers to inherit from the abstract class, decide what to add to it and choose which functions they would like to override; these functions were OnTriggerEnter, OnTriggerStay, OnTriggerExit, OnCollisionEnter, OnCollisionStay, and OnCollisionExit. They would then pass an instance of that class as a parameter when creating a Collider object, and the physics library would handle when these events are called. Tudey could serialize these objects no problem, but deserializing was a different question. I wanted the entire serialization process to be encapsulated in the physics library, but the library does not know what types had been defined outside of it. I ended up creating a very lightweight reflection system that held all the CollisionBehaviour child types that was accessed by their class name.</p>
